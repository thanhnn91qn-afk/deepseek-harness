 ...
 * Browsers restrict `crypto.randomUUID()` to secure contexts (localhost or
 * HTTPS); a LAN origin over plain HTTP does not qualify, but
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  protected mintRpcId(): RpcId {
    ...
    // expose it in a secure context (localhost or HTTPS) — a LAN origin over
    // plain HTTP does not qualify, so every RPC would throw here without the
    // fallback. Node has no secure-context concept and always exposes it.
